...
h1= [1 0.8 0.6 0.4 0.2 zeros(1,5)];
h = fliplr(h1); % impulse-like signal h[n]
m = -10:10;                    % shift range for correlation
shifts = -15:10;               % shifts for animation
...
% GIF setup
gif_filename = 'conv_animation.gif';
delay_time = 0.5;
...
for k = 1:length(shifts)
    ...
    stem([0:length(h1)-1], h1, 'r', 'filled', 'LineWidth',1.2); hold off;
    ...
    plot(curr_shifts+10, real(r_vals), '-o','LineWidth',1.2);
    hold on;
    plot(s+10, real(r_at_s), 'ks','MarkerFaceColor','y','MarkerSize',8);
    hold off;
    xlabel('shift s'); ylabel('Re\{r[s]\}'); grid on;
    title('Running correlation vs shift');
    xlim([min(shifts)+9 max(shifts)+7]);
    ylim([-1,2])
